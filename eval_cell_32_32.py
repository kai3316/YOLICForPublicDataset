...
from model.mobilenet32 import mobilenet_v2
...
# network to data parallel
network = torch.nn.DataParallel(network)
checkpoint = torch.load(
    "/home/kai/Downloads/model__epoch_15_loss_0.058974013.pth")
...
for step, (imgs, label_imgs) in enumerate(val_loader):
    with torch.no_grad():  # (corresponds to setting volatile=True in all variables, this is done during inference to reduce memory consumption)
        ...
        outputs = np.reshape(outputs, (5 * 32 * 64, 1)).flatten()
        labels = np.reshape(labels, (5 * 32 * 64, 1)).flatten()
        # outputs = np.where(outputs >= 0.5, 1, 0)
