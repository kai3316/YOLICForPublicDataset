...
checkpoint = torch.load(
    "/home/kai/Desktop/YOLICForPublicData/mobilenet_v2_cityscapes_64_64_flip/model__epoch_9_loss_0.06400229.pth")
...
for step, (imgs, label_imgs) in enumerate(val_loader):
    with torch.no_grad():  # (corresponds to setting volatile=True in all variables, this is done during inference to reduce memory consumption)
        ...
        outputs = np.reshape(outputs, (5 * 16 * 32, 1)).flatten()
        labels = np.reshape(labels, (5 * 16 * 32, 1)).flatten()
        # outputs = np.where(outputs >= 0.5, 1, 0)
